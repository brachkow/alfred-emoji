...
class EmojiSearch {
  ...
  private loadEmojiData(): void {
    try {
      ...
      // Merge emoji data with shortcodes
      this.emojis = emojiData.map((emoji: any) => ({
        ...emoji,
        shortcodes: shortcodes[emoji.hexcode] ? [shortcodes[emoji.hexcode]] : []
      }));
    } catch (error) {
  ...
  private searchEmojis(query: string): EmojiData[] {
    ...
    for (const emoji of this.emojis) {
      ...
      // Since emojibase doesn't have tags, we'll use the label as the main search field
  ...
  private createAlfredItem(emoji: EmojiData): AlfredItem {
    const shortcodes = emoji.shortcodes?.join(', ') || '';
